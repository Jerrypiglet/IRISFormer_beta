...
png_path = '/data/ruizhu/OR-pngs'
png_path_dest = '/data/ruizhu/ORmini-pngs'
...
for dir_ in tqdm(dirs):
    for scene in scene_list:
        src_path = Path(png_path) / dir_ / scene
        dest_path = Path(png_path_dest) / dir_ / scene
        dest_path.parent.mkdir(parents=True, exist_ok=True)
        if src_path.exists():
            result = shutil.copytree(src_path, dest_path)
            print('Copied from %s to %s'%(src_path, dest_path))
        else:
            print('Skipped %s'%(str(src_path)))
