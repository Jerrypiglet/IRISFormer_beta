    ...
    if if_loss:
        ...
        if 'no' in opt.cfg.MODEL_BRDF.enable_list:
            ...
            if opt.cfg.DATA.if_pad_to_32x:
                normalPred = normalPred[:, :, :opt.cfg.DATA.im_height, :]
                normalBatch = normalBatch[:, :, :opt.cfg.DATA.im_height, :]
                segNormalBatch = segNormalBatch[:, :, :opt.cfg.DATA.im_height, :]
                assert normalPred.shape[-1] == normalBatch.shape[-1] == segNormalBatch.shape[-1] == opt.cfg.DATA.nyud.im_width_padded_to
        ...
        if 'de' in opt.cfg.MODEL_BRDF.enable_list:
            ...
            if opt.cfg.DATA.if_pad_to_32x:
                depthPred = depthPred[:, :, :opt.cfg.DATA.im_height, :]
                segDepthBatch = segDepthBatch[:, :, :opt.cfg.DATA.im_height, :]
                depthBatch = depthBatch[:, :, :opt.cfg.DATA.im_height, :]
                assert depthPred.shape[-1] == segDepthBatch.shape[-1] == depthBatch.shape[-1] == opt.cfg.DATA.nyud.im_width_padded_to
