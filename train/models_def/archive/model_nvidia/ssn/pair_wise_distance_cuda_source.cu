...
// CL- TODO: change this to Mahanobis distance (E-step in EM)
    ...
    if (x_index + spixel_offset_x < 0 || x_index + spixel_offset_x >= num_spixels_w) {
        ...
        for (int c=0; c<channels; c++)
        {
            sum_squared_diff += pow(pixel_features[b * cp + c * num_pixels + p] - 
                spixel_features[b * cs + c * num_spixels + query_spixel_index], 2); // pixel_features, spixel_features size: B-C-N_pix, B-C-N_spix
        }
...
// CL- TODO: adapt backward to Mahanobis distance (E-step in EM)
template <typename scalar_t>
