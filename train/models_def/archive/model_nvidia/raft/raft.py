...
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        if self.args.cfg.MODEL_GMM.RAFT.small:
    ...
    def initialize_flow(self, img, dtype=torch.float32):
        ...
        N, C, H, W = img.shape
        coords0 = coords_grid(N, H//8, W//8, dtype = dtype, device = img.device)
        coords1 = coords_grid(N, H//8, W//8, dtype = dtype, device = img.device)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            fmaps, fmaps_quad, fmaps_half = self.fnet([image1, image2])
        ...
        with autocast(enabled=self.args.mixed_precision):
            cnet, _, _ = self.cnet(image1)
            net, inp = torch.split(cnet, [hdim, cdim], dim=1)
            net = torch.tanh(net)
            inp = torch.relu(inp)

        coords0, coords1 = self.initialize_flow(image1, dtype=inp.dtype)
        ...
        return coords1 - coords0, flow_up, fmaps, fmaps_quad, fmaps_half
            
        # # TODO: also output feature maps for training
