...
def get_abs_indices(init_label_map, num_spixels_width):
    ...
    abs_pix_indices = torch.arange(n_pixel, device=device)[None, None].repeat(b, 9, 1).reshape(b, -1).long()
    abs_spix_indices = (init_label_map[:, None] + relative_spix_indices[None, :, None]).reshape(b, -1).long()
    abs_batch_indices = torch.arange(b, device=device)[:, None, None].repeat(1, 9, n_pixel).reshape(b, -1).long()

    # import ipdb; ipdb.set_trace()
    return torch.stack([abs_batch_indices, abs_spix_indices, abs_pix_indices], 1)
...
def affinity_rel2abs(affinity_matrix, abs_indices, num_spixels, hard_labels=None):
    ...
    optional 
     ...
     OUTPUTS
        B J N
    '''
    # print(affinity_matrix.shape, abs_indices.shape) # torch.Size([12, 9, 76800]) torch.Size([12, 3, 691200])
    assert affinity_matrix.shape[0]==abs_indices.shape[0]
    b = affinity_matrix.shape[0]
    reshaped_affinity_matrix = affinity_matrix.reshape(b, -1) 
    mask = (abs_indices[:, 1:2, :] >= 0)*(abs_indices[:, 1:2, :] < num_spixels)
    # print(abs_indices.shape, mask.shape) # torch.Size([12, 3, 691200]) torch.Size([12, 1, 691200])

    # abs_indices: Bx3x(9HW), 
    # affinity_matrix: Bx9x(HW) B=1, 
    # reshaped_affinity_matrix: (9BHW), B=b
    # print(abs_indices.shape, reshaped_affinity_matrix.shape, mask.shape, mask.repeat(1, 3, 1).shape) # torch.Size([12, 3, 691200]) torch.Size([12, 691200]) torch.Size([3, 691200])
    # print(mask.shape, abs_indices.shape, reshaped_affinity_matrix.shape)

    # print(abs_indices[mask.repeat(1, 3, 1)].reshape(b, 3, -1).shape, reshaped_affinity_matrix[mask.squeeze(1)].reshape(b, -1).shape) # torch.Size([12, 3, 659984]) torch.Size([12, 659984])
    sparse_abs_affinity = torch.sparse_coo_tensor(abs_indices[mask.repeat(1, 3, 1)].reshape(b, 3, -1), reshaped_affinity_matrix[mask.squeeze(1)].reshape(b, -1))
    abs_affinity = sparse_abs_affinity.to_dense().contiguous()  #BxJx(HW), B=b
...
def get_init_params(images, Z_outer, num_spixels_width, num_spixels_height):
    ...
    with torch.no_grad():
        ...
        abs_indices = get_abs_indices(init_label_map, num_spixels_width) # [b, 3, N]
    ...
    abs_affinity = affinity_rel2abs(affinity_m_init, abs_indices, num_spixels) #BxJxN_pix, \Gamma matrix
    abs_affinity = abs_affinity / abs_affinity.sum(1).unsqueeze(1) # normalize 

    T0 = abs_affinity.sum(-1).unsqueeze(-1) #BxJx1
    T1 = torch.matmul(abs_affinity, images.permute(0, 2, 3, 1).reshape(images.shape[0], -1,3)) # BxJx3
    # print(abs_affinity.shape, T0.shape, images.shape, T1.shape, Z_outer.shape) # torch.Size([12, 315, 76800]) torch.Size([12, 315, 1]) torch.Size([12, 3, 240, 320]), torch.Size([12, 76800, 9])
    T2 = torch.matmul(abs_affinity, Z_outer) #BxJx9
    mix = T0/ (height*width) #Jx1
    mu = T1/ T0 #Jx3
    #Get outer mu
    # outer_mu = outer_prod(mu, mu).reshape(-1, 9)
    outer_mu = outer_prod_batch(mu, mu).reshape(mu.shape[0], -1, 9)
    # print(outer_mu.shape, T2.shape, T0.shape) # torch.Size([12, 315, 9]) torch.Size([12, 315, 9]) torch.Size([12, 315, 1])
    #Get sigma
    sigma = T2/T0 - outer_mu #Jx9
...
def ssn3d_iter(
    ...
    # pts_outer = outer_prod(permuted_pixel_features.squeeze(), permuted_pixel_features.squeeze()).reshape(-1, 9) #  #Bx N_pix x C^2
    pts_outer = outer_prod_batch(permuted_pixel_features.squeeze(), permuted_pixel_features.squeeze()) # Bx N_pix x C x C
    pts_outer = pts_outer.reshape(pts_outer.shape[0], pts_outer.shape[1], 9) # Bx N_pix x C^2
    # print(permuted_pixel_features.shape, pts_outer.shape)
    spixel_mu, spixel_cov, mix, init_label_map, abs_indices = get_init_params(pixel_features, pts_outer, num_spixels_width, num_spixels_height) # torch.Size([12, 315, 3]) torch.Size([12, 315, 9]) torch.Size([12, 315, 1]) torch.Size([12, 76800]) torch.Size([3, 8294400])
