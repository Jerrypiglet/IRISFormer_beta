...
def mark_gamma_boundary(img_rgb, gamma):
    '''
    inputs:
    img_rgb - HxWx3 rgb image
    gamma - HxWxJ gamma image

    output:
    img_with_boundary - HxWx3 rgb image with mared boundaries

    '''
    label_img = torch.argmax(gamma, dim=-1).detach().cpu().numpy()
    img_with_boudary = mark_label_boundary(img_rgb.cpu().numpy(),
                                           label_img)

    return img_with_boudary


def mark_label_boundary(img, label_img):
    '''
    mark boundary in the label_img

    input:
    img- HxW or HxWx3 numpy array or 3xHxW or HxW tenor
    label_img- HxW numpy array or tensor

    output:
    img_with_boundary - HxWx3 rgb image with marked boundaries
    '''
    ...
    if isinstance(label_img, torch.Tensor) and isinstance(img, torch.Tensor):
        img_ = img.squeeze()
        img_ = rescale(img_)
        if img_.shape[0] == 3:
            img_= img_.permute(2,0,1).contiguous().detach().cpu().numpy()
            return mark_boundaries( img_, label_img.detach().cpu().numpy() )
        else:
            return mark_boundaries( img_.detach().cpu().numpy(), label_img.detach().cpu().numpy() )
            
    else:
        return mark_boundaries( rescale(img), label_img )