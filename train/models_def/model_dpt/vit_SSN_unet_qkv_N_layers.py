...
def forward_vit_SSN_qkv_yogo_N_layers(opt, pretrained, x, input_dict_extra={}, hooks=[]):
    b, c, h, w = x.shape
    ...
    layer_dict = {}
    for layer_idx in hooks:
        layer_dict['%d'%layer_idx] = flex_return_dict['im_feat_dict']['im_feat_%d'%layer_idx]
        if if_print:
            print('---', layer_idx, layer_dict['%d'%layer_idx].shape)
        ...
        #     print(pretrained.act_postprocess3[3:])act_postprocess_dict
        # print(pretrained.act_postprocess4[3:])

    for layer_idx in hooks:
        act_postprocess = pretrained.act_postprocess_dict['%d'%layer_idx]
        layer_dict['%d'%layer_idx] = act_postprocess[3:](layer_dict['%d'%layer_idx])
        if if_print:
            print('--->', layer_idx, layer_dict['%d'%layer_idx].shape)
