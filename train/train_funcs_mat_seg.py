...
def process_mat_seg(input_dict, output_dict, loss_dict, opt, time_meters):
    ...
    loss_dict['loss_mat_seg-push'] = torch.mean(torch.stack(loss_push_list))
    loss_dict['loss_mat_seg-binary'] = torch.mean(torch.stack(loss_binary_list))
...
def get_input_dict_mat_seg_combine(data_batch, opt):
