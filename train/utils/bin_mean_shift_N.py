...
class Bin_Mean_Shift_N(nn.Module):
    ...
    def generate_seed(self, point, bin_num):
        ...
        min_xyz_list = point.min(dim=0)[0]
        max_xyz_list = point.max(dim=0)[0]

        start_xyz_list, end_xyz_list = [], []
        xyz_list = []

        for min_xyz, max_xyz in zip(min_xyz_list, max_xyz_list):
            start_xyz, end_xyz = get_start_end(min_xyz.item(), max_xyz.item(), bin_num)
            start_xyz_list.append(start_xyz)
            end_xyz_list.append(end_xyz)

        xyz_repeat_list = []
        for idx, (start_xyz, end_xyz) in enumerate(zip(start_xyz_list, end_xyz_list)):
            view_list = [1] * len(start_xyz_list)
            view_list[idx] = bin_num
            xyz = torch.linspace(start_xyz, end_xyz, bin_num).view(view_list)
            repeat_list = [bin_num] * len(start_xyz_list)
            repeat_list[idx] = 1
    ...
    def cal_distance_matrix(self, point_a, point_b):
        ...
        a_repeat = point_a.repeat(1, n).view(n * m, self.embedding_dims)                  # (n*m, 2)
