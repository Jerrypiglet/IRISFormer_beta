...
def get_layout_bdb_sunrgbd(bins_tensor, lo_ori_reg, lo_ori_cls, centroid_reg, coeffs_reg, if_return_bdb=True):
    """
    ...
    if if_return_bdb:
        ...
        return basis, coeffs_reg, centroid_reg
...
def get_bdb_evaluation(bins_tensor, ori_cls_gt, ori_reg_result, centroid_cls_gt, centroid_reg_result, size_cls_gt,
    ...
    bdb_output = [{'basis': basis[i, :, :].cpu().detach().numpy(), 'coeffs': coeffs[i, :].cpu().detach().numpy(),
                   'centroid': centroid[i, :].squeeze().cpu().detach().numpy(), 'classid': class_id[i].cpu().detach().numpy()} for i in
                  range(n)]
    if not return_bdb:
        return bdb_output
    else:
