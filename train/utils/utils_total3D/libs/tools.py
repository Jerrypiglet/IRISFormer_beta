...
def get_world_R(cam_R, check_assume=True):
    '''
    ...
    # return world_R_
    # print('---', yaw, world_R, world_R_)
    cam_assertion = True
    if check_assume:
        cam_assertion = np.max(np.abs(world_R - world_R_)) < 1e-5

    return world_R, cam_assertion
...
def camera_cls_reg_sunrgbd(cam_R, bin, sample_id=None):
    '''
