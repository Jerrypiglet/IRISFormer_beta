...
class Bin_Mean_Shift_3(nn.Module):
    def __init__(self, train_iter=5, test_iter=10, bandwidth=0.5, device='cpu', invalid_index=255):
        super(Bin_Mean_Shift_3, self).__init__()
    ...
    def generate_seed(self, point, bin_num):
        ...
        min_x, min_y, min_z = point.min(dim=0)[0]
        max_x, max_y, max_z = point.max(dim=0)[0]
        ...
        x = torch.linspace(start_x, end_x, bin_num).view(bin_num, 1, 1)
        y = torch.linspace(start_y, end_y, bin_num).view(1, bin_num, 1)
        z = torch.linspace(start_z, end_z, bin_num).view(1, 1, bin_num)

        x_repeat = x.repeat(1, bin_num, bin_num).view(-1, 1)
        y_repeat = y.repeat(bin_num, 1, bin_num).view(-1, 1)
        z_repeat = z.repeat(bin_num, bin_num, 1).view(-1, 1)

        return torch.cat((x_repeat, y_repeat, z_repeat), dim=1).to(self.device)
    ...
    def cal_distance_matrix(self, point_a, point_b):
        ...
        a_repeat = point_a.repeat(1, n).view(n * m, 3)                  # (n*m, 2)
    ...
    def merge_center(self, seed_point, bandwidth=0.25):
        ...
        is_center = np.ones(n, dtype=np.bool_)
    ...
    def bin_shift(self, prob, embedding, gt_seg, bandwidth):
        """
        discrete seeding mean shift in training stage
        :param prob: tensor with size (1, h, w) indicate probability of being plane
        :param embedding: tensor with size (2, h, w)
        ...
        return segmentation, sampled_segmentation, center, sample_prob, seg[rand_index].view(-1, 1)


    # def forward_old(self, logit, embedding, param, gt_seg):
    ...
    def forward(self, logit, embedding, gt_seg):
        batch_size, c, h, w = embedding.size()
        assert(c == 2)
        ...
        for b in range(batch_size):
            segmentation, sample_segmentation, center, prob, sample_seg = \
                self.bin_shift(torch.sigmoid(logit[b]), embedding[b], gt_seg[b], self.bandwidth)
        ...
        return segmentations, sample_segmentations, centers, sample_probs, sample_gt_segs
