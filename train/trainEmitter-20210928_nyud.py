...
from dataset_openrooms_OR_scanNetPose_light_20210928_nyud import nyud, collate_fn_nyud
...
parser.add_argument("--if_train_nyud", type=str2bool, nargs='?', const=True, default=True)
...
# >>>>>>>>>>>>> DATASET
transforms_train_semseg_nyud = get_transform_semseg('train', opt, pad_op_override=opt.pad_op_nyud)
transforms_val_semseg_nyud = get_transform_semseg('val', opt, pad_op_override=opt.pad_op_nyud)
transforms_train_matseg_nyud = get_transform_matseg('train', opt, pad_op_override=opt.pad_op_nyud)
transforms_val_matseg_nyud = get_transform_matseg('val', opt, pad_op_override=opt.pad_op_nyud)
transforms_train_resize_nyud = get_transform_resize('train', opt, pad_op_override=opt.pad_op_nyud)
transforms_val_resize_nyud = get_transform_resize('val', opt, pad_op_override=opt.pad_op_nyud)


if opt.if_train:
    nyud_dataset_train = nyud(opt, 
        transforms_fixed = transforms_val_resize_nyud, 
        transforms_semseg = transforms_train_semseg_nyud, 
        transforms_matseg = transforms_train_matseg_nyud, 
        transforms_resize = transforms_train_resize_nyud, 
        cascadeLevel = opt.cascadeLevel, split = 'train', if_for_training=True, logger=logger)
    nyud_loader_train, _ = make_data_loader_to_use(
        opt,
        nyud_dataset_train,
        is_train=True,
        start_iter=0,
        logger=logger,
        collate_fn=collate_fn_nyud, 
)

if opt.if_val:
    nyud_dataset_val = nyud(opt, 
        transforms_fixed = transforms_val_resize_nyud, 
        transforms_semseg = transforms_val_semseg_nyud, 
        transforms_matseg = transforms_val_matseg_nyud,
        transforms_resize = transforms_val_resize_nyud, 
        cascadeLevel = opt.cascadeLevel, split = 'val', if_for_training=False, load_first = -1, logger=logger)
    nyud_loader_val, _ = make_data_loader_to_use(
        opt,
        nyud_dataset_val,
        is_train=False,
        start_iter=0,
...
if opt.if_vis:
    nyud_dataset_val_vis = nyud(opt, 
        transforms_fixed = transforms_val_resize_nyud, 
        transforms_semseg = transforms_val_semseg_nyud, 
        transforms_matseg = transforms_val_matseg_nyud, 
        transforms_resize = transforms_val_resize_nyud, 
        cascadeLevel = opt.cascadeLevel, split = 'val', task='vis', if_for_training=False, load_first = opt.cfg.TEST.vis_max_samples, logger=logger)
    nyud_loader_val_vis, batch_size_val_vis = make_data_loader(
        opt,
        nyud_dataset_val_vis,
        is_train=False,
        start_iter=0,
        logger=logger,
        workers=2,
...
from train_funcs_joint_all_nyud import get_labels_dict_joint_nyud, val_epoch_joint_nyud, vis_val_epoch_joint_nyud, forward_joint_nyud, get_time_meters_joint_nyud
...
if not opt.if_train:
    ...
    if opt.if_val:
        ...
        val_params.update({'nyud_dataset_val': nyud_dataset_val})
    ...
    for epoch_0 in list(range(opt.cfg.SOLVER.max_epoch)):
        ...
        time_meters_nyud = get_time_meters_joint_nyud()
        ...
        start_iter = tid_start + len(nyud_loader_train) * epoch_0
        ...
        # for i, data_batch in tqdm(enumerate(nyud_loader_train)):
        for i, (data_batch_nyud, data_batch) in enumerate(zip(cycle(nyud_loader_train), brdf_loader_train)):
        # for i, data_batch in tqdm(enumerate(nyud_loader_train)):
            ...
            if opt.if_train_OR:
                ...
                # else:
                if opt.if_train_nyud:
                    if opt.distributed:
                        model.module.unfreeze_BRDF_except_depth_normal(if_print=False)
                    else:
                        model.unfreeze_BRDF_except_depth_normal(if_print=False)
            ...
            if opt.if_train_nyud:
                ...
                labels_dict_nyud = get_labels_dict_joint_nyud(data_batch_nyud, opt)

                time_meters_nyud['data_to_gpu'].update(time.time() - ts_iter_start)
                time_meters_nyud['ts'] = time.time()
                ...
                # else:
                if opt.distributed:
                    model.module.freeze_BRDF_except_depth_normal(if_print=False)
                else:
                    model.freeze_BRDF_except_depth_normal(if_print=False)
                ...
                loss_keys_backward.append('loss_nyud-ALL')
                loss_keys_print.append('loss_nyud-ALL')
                if 'no' in opt.cfg.MODEL_BRDF.enable_list:
                    loss_keys_print.append('loss_nyud-normal')
                if 'de' in opt.cfg.MODEL_BRDF.enable_list:
                    loss_keys_print.append('loss_nyud-depth')
                ...
                # else:
                optimizer.step()
                time_meters_nyud['backward'].update(time.time() - time_meters_nyud['ts'])
                time_meters_nyud['ts'] = time.time()

                if opt.is_master:
                    loss_keys_print = [x for x in loss_keys_print if 'ALL' in x] + [x for x in loss_keys_print if 'ALL' not in x]
                    logger_str = '[nyud] Epoch %d - Tid %d -'%(epoch, tid) + ', '.join(['%s %.3f'%(loss_key, loss_dict_reduced[loss_key]) for loss_key in loss_keys_print])
