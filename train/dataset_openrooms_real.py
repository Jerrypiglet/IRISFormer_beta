...
class openrooms_real(data.Dataset):
    ...
    def __getitem__(self, ind):
        ...
        batchDict.update({'image_transformed': image_transformed, 'im_not_hdr': im_not_hdr, 'im_uint8': im_uint8})
                # if self.transform is not None and not self.opt.if_hdr:
        batchDict.update({'image_transformed': image_transformed, 'im_semseg_transformed_trainval': image_transformed, 'im_not_hdr': im_not_hdr, 'im_fixedscale_SDR': im_not_hdr, 'im_uint8': im_uint8})
        if self.split != 'train':
            batchDict.update({'image_transformed_fixed': image_transformed, 'im_fixedscale_SDR_uint8': im_uint8})
