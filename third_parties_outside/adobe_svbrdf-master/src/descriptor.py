...
class FeatureMaskLoss(th.nn.Module):

    def __init__(self, dir, w):
        super(FeatureMaskLoss, self).__init__()
    ...
    def forward(self, x, mask):
        ...
        result = []
        for i, feature in enumerate(outputs):
            mask_i = F.interpolate(mask, scale_factor=self.scale[i])
            feature_mask = th.masked_select(feature, mask_i > 0.5)

            result.append(feature_mask * self.weights[i])
