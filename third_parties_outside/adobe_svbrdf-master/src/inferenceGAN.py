...
def tex2pngConvert(tex, fn, isVertical=False):
    ...
    ######## Optimization-based Conversion
    basecolor = torch.from_numpy(albedo).cuda().requires_grad_()
    #basecolor = torch.from_numpy(np.random.uniform(0, 1, albedo.shape).astype(np.float32)).cuda().requires_grad_()
    metallic = torch.from_numpy(0.5*np.ones(albedo[:,:,0].shape).astype(np.float32)).cuda().requires_grad_()
    diffuseTarget = torch.from_numpy(albedo).cuda()
    specularTarget = torch.from_numpy(specular).cuda()
    opBasecolor = Adam([basecolor], lr=1e-2, betas=(0.5, 0.999) )
    schBasecolor = lr_scheduler.StepLR(opBasecolor, step_size=1, gamma=0.5)
    opMetallic = Adam([metallic], lr=1e-1, betas=(0.5, 0.999) )
    schMetallic = lr_scheduler.StepLR(opMetallic, step_size=1, gamma=0.5)
    # Loss function
    ...
    for it in range(100):
        for _ in range(2):
            # Update Basecolor
            opBasecolor.zero_grad()
            lossDiffuse = criterion(basecolor - basecolor * metallic.unsqueeze(2), diffuseTarget)
            lossSpecular = criterion(0.04 - 0.04 * metallic.unsqueeze(2) + basecolor * metallic.unsqueeze(2), specularTarget)
            loss = lossDiffuse + lossSpecular
            loss.backward()
            opBasecolor.step()
        loss1 = loss.item()
        print('[%4d, 1000] After Optimized Basecolor, Loss: %.8f (Diffuse: %.8f, Specular: %.8f)' % (it, loss.item(), lossDiffuse.item(), lossSpecular.item()))
            
        for _ in range(2):
            # Update Metallic
            opMetallic.zero_grad()
            lossDiffuse = criterion(basecolor - basecolor * metallic.unsqueeze(2), diffuseTarget)
            lossSpecular = criterion(0.04 - 0.04 * metallic.unsqueeze(2) + basecolor * metallic.unsqueeze(2), specularTarget)
            loss = lossDiffuse + lossSpecular
            loss.backward()
            opMetallic.step()
        loss2 = loss.item()
        print('[%4d, 1000] After Optimized Metallic, Loss: %.8f (Diffuse: %.8f, Specular: %.8f)' % (it, loss.item(), lossDiffuse.item(), lossSpecular.item()))

        if np.abs(loss1 - loss2) < 1e-9:
            if loss2 < 1e-5:
                break
            else:
                print('Reduce half learning rate!')
                schBasecolor.step()
                schMetallic.step()

            
    basecolor = basecolor.clamp(0, 1)
    metallic = metallic.clamp(0, 1)
    basecolor = basecolor.cpu().detach().numpy()
    metallic = metallic.cpu().detach().numpy()
    ################
...
def saveTex(tex, save_dir, tmp_dir, idx):
    ...
    png = tex2pngConvert(tex, fn, isVertical=False)
    return png
...
def optim(args):
    ...
    for idx in range(10):
        ...
        png = saveTex(texture_pre, out_this_dir, out_this_tmp_dir, idx)
        if idx > 0:
            pngFull = gyConcatPIL_v(pngFull, png)
        else:
            pngFull = png
    
    pngFull.save(os.path.join(out_this_dir, 'texFull.png'))
    assert(False)
