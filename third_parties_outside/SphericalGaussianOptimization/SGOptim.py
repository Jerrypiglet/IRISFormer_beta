...
class SGEnvOptim():
    def __init__(self, isCuda = True, gpuId = 0, niter = 10, envNum = 19200,
        ...
        # [!!!] self.ls is the grid vectors in 3D (local SG coords); SG works by taking the dot product between a grid vector and the SG center axis
        ...
        if param is not None:
            assert self.param.shape == param.shape
            self.param += param
    ...
    def renderSG(self, theta, phi, lamb, weight, axis_in=None):
        ...
        #     camx_reshape = camx.view(self.envNum, 1, 1, 1, 1, 3)
        #     camy_reshape = camy.view(self.envNum, 1, 1, 1, 1, 3)
        #     camz_reshape = camz.view(self.envNum, 1, 1, 1, 1, 3)
        #     axis = axis[:, :, :, :, :, 0:1] * camx_reshape + axis[:, :, :, :, :, 1:2] * camy_reshape + axis[:, :, :, :, :, 2:3] * camz_reshape
        
        # self.ls: [1, 1, 1, 256, 512, 3]
        # axis: [19200, 12, 1, 1, 1, 3]; axis is in ls coords
    ...
    def optimize(self, envmap, mask = None ):
        ...
        for i in range(0, self.niter ):
            ...
            def closure( ):
                ...
                # print(theta.shape, phi.shape, lamb.shape, weight.shape )
