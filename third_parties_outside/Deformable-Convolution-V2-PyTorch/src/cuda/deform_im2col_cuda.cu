...
at::Tensor
deform_im2col_cuda_forward(const at::Tensor &input,
                    ...
                    const int im2col_step)
{
    ...
    AT_ASSERTM(im2col_step_ == 1, "only support im2col_step == 1"); // [Rui] only support im2col_step == 1
    AT_ASSERTM(group == 1, "only support group == 1"); // [Rui] only support group == 1
    AT_ASSERTM(deformable_group == 1, "only support deformable_group == 1"); // [Rui] only support deformable_group == 1


    AT_ASSERTM(batch % im2col_step_ == 0, "batch(%d) must divide im2col_step(%d)", batch, im2col_step_);

    AT_ASSERTM(channels % group == 0, "channels(%d) must divide group(%d)", channels, group);
    ...
    auto columns_n = at::empty({batch/im2col_step_, channels * kernel_h * kernel_w, batch_n * height_out * width_out}, input.options());
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        auto columns = at::empty({channels * kernel_h * kernel_w, batch_n * height_out * width_out}, input.options());
        AT_DISPATCH_FLOATING_TYPES(input.type(), "deform_im2col_forward_cuda", ([&] {
        ...
        // ==> Rui
        // auto columns_g = columns_n.select(0, n).view({channels * kernel_h * kernel_w, batch_n * height_out * width_out});
        columns_n.select(0, n) = columns; 
        // <== Rui
...
std::vector<at::Tensor> deform_im2col_cuda_backward(const at::Tensor &input,
                                             ...
                                             const int im2col_step)
{
    ...
    const int HWC = grad_output.size(1); // kernel_h * kernel_w * channels
    const int N_locations = grad_output.size(2); // N (number of conv locations) == batch_n * height_out * width_out
    AT_ASSERTM(HWC == kernel_h * kernel_w * channels, "HWC i.e. grad_output.size(1) != kernel_h * kernel_w * channels");
    ...
    const int batch_n = im2col_step_;
    AT_ASSERTM(N_locations == batch_n * height_out * width_out, "N_locations i.e. grad_output.size(2) != batch_n * height_out * width_out");
    const int per_input_size = channels * height * width;
    const int per_offset_size = offset.size(1) * offset.size(2) * offset.size(3);
    // auto grad_output_n = grad_output.view({batch/im2col_step_, batch_n, channels_out, height_out, width_out});
    auto grad_columns_n = grad_output.view({batch/im2col_step_, channels * kernel_h * kernel_w, batch_n * height_out * width_out});

    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        // auto grad_output_g = grad_output_n.select(0, n).view({batch_n, group, channels_out/group, height_out, width_out});
        // auto grad_output_g = grad_output_n.select(0, n).view({batch_n, group, HWC/group, height_out});
        ...
        auto columns = grad_columns_n.select(0, n);

        AT_DISPATCH_FLOATING_TYPES(input.type(), "deform_im2col_backward_cuda", ([&] {
