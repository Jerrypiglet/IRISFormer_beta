...
from functions.deform_conv_func import DeformConvFunction, DeformIm2colFunction
...
class DeformIm2col(nn.Module):

    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True):
        super(DeformIm2col, self).__init__()
    ...
    def forward(self, input, offset):
        assert 2 * self.deformable_groups * self.kernel_size[0] * self.kernel_size[1] == \
            offset.shape[1]
        return DeformIm2colFunction.apply(input, offset,
                                                   self.kernel_size, 
                                                   self.stride,
                                                   self.padding,
                                                   self.dilation,
...
_DeformIm2col = DeformIm2colFunction.apply