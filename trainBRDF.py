...
# Rui
parser.add_argument('--isMatMaskInput', action='store_true', help='using matMask as additional input')
...
opt.experiment = 'results/' + opt.experiment
print('----opt.experiment', opt.experiment)
os.system('mkdir {0}'.format(opt.experiment) )
os.system('cp train %s' % opt.experiment )
...
for epoch in list(range(opt.epochIdFineTune+1, opt.nepoch) ):
    ...
    for i, dataBatch in enumerate(brdfLoader):
        ...
        matMask_cpu = dataBatch['matMask']
        matMaskBatch = Variable(matMask_cpu ).cuda()
