    ...
    for epoch in list(range(opt.epochIdFineTune+1, opt.nEpoch)):
        ...
        for i, dataBatch in enumerate(brdfLoader):
            ...
            if i % opt.saveStep == 0:
                ...
                vutils.save_image(inputs, os.path.join(
                        imgSavePath, 'train_inputs_%04d.png' % wStep))
                ...
                vutils.save_image(matsGT, os.path.join(
                        imgSavePath, 'train_matGT_%04d.png' % wStep))
                ...
                vutils.save_image(matsPred, os.path.join(
                        imgSavePath, 'train_matPred_%04d.png' % wStep))
                print('4-3. time : {}'.format(timeit.default_timer() - startTime) )
                albedoBatch = dataBatch['albedo'] ** (1.0/2.2)
                normalBatch = (dataBatch['normal'] + 1) * 0.5
                roughBatch = (dataBatch['rough'] + 1) * 0.5
                matsScene = torch.cat(
                    [dsF(albedoBatch), dsF(normalBatch), to3C(dsF(roughBatch) )], dim=2)
                writer.add_image('matsScene', vutils.make_grid(
                    matsScene, nrow=8), wStep)
                vutils.save_image(matsScene, os.path.join(
                        imgSavePath, 'train_matScene_%04d.png' % wStep))
