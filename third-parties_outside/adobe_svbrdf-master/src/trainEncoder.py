...
def train(opt):
    ...
    if not os.path.exists(modelSavePath):
        os.system('mkdir -p {}'.format(modelSavePath))
    ...
    for epoch in list(range(opt.epochIdFineTune+1, opt.nEpoch)):
        ...
        for i, dataBatch in enumerate(brdfLoader):
            ...
            if opt.mode == 'cs':
                ...
                styleErr = l2Loss(output['material'], matStyleBatch)
                loss = styleErr

            loss.backward()
            opNet.step()
            #print('3. time : {}'.format(timeit.default_timer() - startTime) )
                # matOptPath = osp.join(
            ...
            if i % opt.printStep == (opt.printStep-1):
                ...
                if opt.mode == 'cs':
                    ...
                    total = 0
                    correct = 0
                    printMSG('[Epoch %d/%d, %5d/%5d] Class loss: %.4f accu: %.2f %% Scale loss: %.4f Total loss: %.4f' %
                            (epoch, opt.nEpoch, i + 1, len(brdfLoader), classErr.item(), classAccu, scaleErr.item(), loss.item()), trainingLog)
                    writer.add_scalar('CS/Train/MatClassLoss', classErr.item(), wStep)
                    writer.add_scalar('CS/Train/ScalesLoss', scaleErr.item(), wStep)
                    writer.add_scalar('CS/Train/MatClass+ScalesLoss', loss.item(), wStep)
                    writer.add_scalar('CS/Train/MatClassAccuracy', classAccu, wStep)
                elif opt.mode == 'w':
                    printMSG('[Epoch %d/%d, %5d/%5d] Style loss: %.4f ' %
                            (epoch, opt.nEpoch, i + 1, len(brdfLoader), styleErr.item()), trainingLog)
                    writer.add_scalar('W/Train/MatStyleLoss', styleErr.item(), wStep)
            ...
            if i % opt.saveStep == 0:
                ...
                writer.add_image('%s/Train/inputs' % opt.mode.upper(), vutils.make_grid(inputs[:8], nrow=8), wStep)
                vutils.save_image(inputs, os.path.join(imgSavePath, 'train_inputs_%04d.png' % wStep))
                ...
                if opt.mode == 'cs':
                    ...
                    matsGT = getMatFromStyle(genObj, matStyleBatch, opt.mode)
                    matsPred = getMatFromStyle(genObj, output['material'].detach(), opt.mode)
                writer.add_image('%s/Train/matsGT' % opt.mode.upper(), vutils.make_grid(matsGT[:8], nrow=8), wStep)
                vutils.save_image(matsGT, os.path.join(imgSavePath, 'train_matGT_%04d.png' % wStep))
        ...
        # Save model
        torch.save(net.state_dict(), os.path.join(modelSavePath, 'net_epoch_{}.pth'.format(epoch)))
        msg = 'Model saved at {}'.format(os.path.join(modelSavePath, 'net_epoch_{}.pth'.format(epoch)))
        printMSG(msg, trainingLog)
        msg = 'Training time : {}'.format(timeit.default_timer() - startTime)
        printMSG(msg, trainingLog)
        trainingLog.close()
