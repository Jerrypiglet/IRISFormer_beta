...
def optim(args):
    ...
    #textures_ref, _ = png2tex(args.in_dir)
    textures_ref = dir2tex(args.in_dir)
    ...
    #lossObj = LossesMaps(args, texture_init, textures_ref)
    ...
    if args.optim_latent:
        if args.sub_epochs[0] > 0:
            th.save(latent_best, os.path.join(out_this_dir, 'optim_latent.pt'))
        if args.sub_epochs[1] > 0:
            th.save(noise_best, os.path.join(out_this_dir, 'optim_noise.pt'))
        ...
        #print(th.load(os.path.join(out_this_dir, 'optim_latent.pt')).size())
        #print(th.load(os.path.join(out_this_dir, 'optim_noise.pt'))[-1].size())
    png = saveTexAsPNG(texture_best, os.path.join(out_this_dir, 'texOpt.png') )
    png = saveTexAsPNG(textures_ref, os.path.join(out_this_dir, 'texOri.png') )
    #tex_best = updateTextureFromLatent(genObj_best, args.gan_latent_type, latent_best)
    #png = saveTexAsPNG(tex_best, os.path.join(out_this_dir, 'texOpt_2.png') )
