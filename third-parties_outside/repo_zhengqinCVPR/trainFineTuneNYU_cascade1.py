...
parser.add_argument('--nepoch', type=int, default=3, help='the number of epochs for training')
...
if opt.experiment is None:
    opt.experiment = 'check_cascadeNYU1'
os.system('mkdir {0}'.format(opt.experiment) )
os.system('cp *.py %s' % opt.experiment )

if opt.experimentBRDF0 is None:
    opt.experimentBRDF0 = 'check_cascadeNYU0'
...
for epoch in list(range(opt.nepochFineTune, opt.nepoch) ):
    ...
    for i, trainBatch in enumerate(brdfLoader):
        ...
        axisPred.requires_grad = False
        lambPred.requires_grad = False
        weightPred.requires_grad = False
        ...
        NYUBatch['albedoPre'] = albedoPred.detach()
        NYUBatch['normalPre'] = 0.5 * (normalPred.detach() + 1)
        NYUBatch['roughPre'] = 0.5 * (roughPred.detach() + 1)
        NYUBatch['depthPre'] = depthPred.detach()
        NYUBatch['diffusePre'] = diffusePred.detach()
        NYUBatch['specularPre'] = specularPred.detach()
        NYUBatch['envmapsPre'] = envmapsPred.detach()
        ##################################################################################################

        albedoPair, normalPair, roughPair, depthPair,  \
        = wnyu.wrapperNYU(NYUBatch, opt, encoder1, \
        albedoDecoder1, normalDecoder1, roughDecoder1, depthDecoder1 )
        ...
        if j == 1 or j% 1000 == 0:
            vutils.save_image( ( (imBatch)**(1.0/2.2) ).data,
                    '{0}/{1}_imNYU.png'.format(opt.experiment, j) )
