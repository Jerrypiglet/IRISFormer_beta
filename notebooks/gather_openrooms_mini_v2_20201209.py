...
dest_path = '/home/ruizhu/Documents/Projects/semanticInverse/dataset/mini'
...
for dir_ in dirs:
    for scene in scene_list:
        ...
        dst_path = Path(dest_path) / dir_ / scene
        dst_path.parent.mkdir(parents=True, exist_ok=True)
        print('Copying from %s to %s'%(src_path, dst_path))
        result = shutil.copytree(src_path, dst_path)
        print('Done.', os.listdir(result))
...
for split in ['train', 'val']:
    ...
    for scene_name in tqdm(scene_names_split):
        ...
        if len(frame_paths_dict['im_RGB']) == 0:
            print('Split %s: No image files for scene %s. skipped.'%(split, scene_name))
            continue
